...
def get_asset_pairs():
    ...
    progressbar.print(0, size=10, msg='Downloading asset pairs')
    if not cache_asset_pairs:
        cache_asset_pairs = request.request('asset pairs')
    asset_pairs = cache_asset_pairs
    progressbar.erase()
    progressbar.print(100, size=10, msg='Downloading asset pairs')
...
def get_assets():
    global cache_assets
    progressbar.print(0, size=10, msg='Downloading assets')
    if not cache_assets:
        cache_assets = request.request('assets')
    assets = cache_assets
    progressbar.erase()
    progressbar.print(100, size=10, msg='Downloading assets')
    return assets
