...
def get_open_orders():
    ...
    for o_id, v in open_orders['open'].items():
        o_date = datetime.datetime.fromtimestamp(
            int(v['opentm'])).strftime('%Y-%m-%d_%H:%M:%S')
        order_line = (' '.join([o_id, v['descr']['order'], o_date])).split()
        del order_line[4]
        table.append(order_line)

    table_headers = [
        'Open Order #', 'Type', 'Volume', 'Pair',
        'Method', 'Price', 'Created']
...
def get_closed_orders():
    ...
    for o_id, v in closed_orders.items():
        ...
        o_date = datetime.datetime.fromtimestamp(
            ...
            int(v['closetm'])).strftime('%Y-%m-%d_%H:%M:%S')
        try:
    ...
    table_headers = [
        'Closed Order #', 'Type', 'Volume', 'Pair',
        'Method', 'Price', 'Cost', 'Close Time']
    return table, table_headers


if __name__ == '__main__':
    open_orders, open_orders_headers = get_open_orders()
    open_orders = sorted(open_orders, key=lambda x: x[-1], reverse=True)
    closed_orders, closed_orders_headers = get_closed_orders()
    closed_orders = sorted(closed_orders, key=lambda x: x[-1], reverse=True)
    import tabulate
    print(tabulate.tabulate(open_orders, headers=open_orders_headers))
    # print(tabulate.tabulate(closed_orders, headers=closed_orders_headers))
